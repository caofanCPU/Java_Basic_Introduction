...
1. 成员变量(静态变量，通过类.即可访问，参照左边定义类型；
           非静态变量，public权限时也参照左边定义类型，private权限时是通过getter方法覆盖，运行时参照右边)
...
编译时：参考左边定义类中是否有调用的函数。有，编译通过。没有，编译失败。

运行时：参考的是右边创建实例是否有调用的函数。
...
编译时：参考的是左边定义类中是否有调用的函数。

运行时：参考的是左边定义类中是否有调用的函数。
